...
if input("Do you want to delete the metadata for skatt concepts from conceptHarvester (y/n) ?") in ["y", "yes", "yep", "roger", "jada", "ja", "j", "yeah", "can confirm", "yessir", "yessiree"]:
    delete_result = db.conceptMeta.delete_many({"_id": {'$regex': "http://begrepskatalogen/begrep/", '$options': 'i'}})
    if delete_result:
        print("Successfully deleted metadata for skatt concepts" + str(delete_result.deleted_count))
    else:
