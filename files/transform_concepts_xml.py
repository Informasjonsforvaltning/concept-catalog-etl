...
def transform_concept(concept):
    transformed_concept = {
        ...
        "anbefaltTerm": {
            "navn": {
                "nb": concept["preflabel"].get("no"),
                "en": concept["preflabel"].get("en"),
                "nn": concept["preflabel"].get("nn")
            }
        },
        "definisjon": {
            "tekst": {
...
def mapkildetype(kildetype, begrep_id):
    if kildetype is None:
        return "EGENDEFINERT"
    elif kildetype.lower() == "sitat fra kilde":
...
with open('skatt_concepts.json', 'w', encoding="utf-8") as outfile:
